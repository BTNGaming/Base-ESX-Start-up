...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName, amount)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage' and itemName ~= 'antibiotico' and itemName ~= 'antibioticorosacea' and itemName ~= 'sciroppo') then
		print(('[esx_ambulancejob] [^2INFO^7] "%s" attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
	...
	if xPlayer.canCarryItem(itemName, amount) then
		...
		if Config.EnableJobLogs == true then
			TriggerEvent('esx_joblogs:AddInLog',"ambulance" ,"getitem" ,xPlayer.name ,itemName)
		end
	else
